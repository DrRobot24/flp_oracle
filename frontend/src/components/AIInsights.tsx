...
export function AIInsights({ homeTeam, awayTeam, results, homeWave, awayWave, homeNews = [], awayNews = [] }: AIInsightsProps) {
    ...
    // 1b. News Impact Insights
    [...homeNews, ...awayNews].forEach(news => {
        // const isHome = news.teamName === homeTeam
        if (news.category === 'injury') {
            insights.push({
                icon: '🚑',
                text: `${news.teamName}: Essenziale assenza (${news.rawText})`,
                confidence: 'high'
            })
        } else if (news.category === 'motivation') {
            insights.push({
                icon: '🔥',
                text: `${news.teamName}: Fattore motivazionale attivo!`,
                confidence: 'medium'
            })
        } else if (news.sentiment < -0.3) {
            insights.push({
                icon: '⚠️',
                text: `${news.teamName}: News negative recenti`,
                confidence: 'medium'
            })
        }
    })
