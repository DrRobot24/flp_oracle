...
export function Dashboard() {
  ...
  useEffect(() => {
    async function updateStats() {
      ...
      const homeStats = await api.getTeamStats(params.homeTeam)
      const awayStats = await api.getTeamStats(params.awayTeam)
  ...
  const handleCalculate = async () => {
    ...
    if (trajectory.length > 0) {
      ...
      // ---------------------------------------------------------
      // FFT WAVE ANALYSIS 🌊 - Find Hidden Patterns
      // ---------------------------------------------------------
      // Get more match history for better FFT resolution
      const homeStats = await api.getTeamStats(params.homeTeam, 20)
      const signal = matchesToSignal(homeStats.recentMatches, params.homeTeam)
      const wave = analyzeFormWave(signal, 3)
      setWaveAnalysis(wave)
    } else {
    ...
    <div className="min-h-screen bg-background p-4 md:p-8 font-sans text-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-8">
        ...
        <div className="lg:col-span-4 space-y-4 md:space-y-6">
          ...
          {/* NEWS PANEL 📰 */}
          <NewsPanel homeTeam={params.homeTeam} awayTeam={params.awayTeam} />
        </div>
        ...
        <div className="lg:col-span-8 space-y-4 md:space-y-6">
          ...
          {/* FFT WAVE ANALYSIS CHART 🌊 */}
          {waveAnalysis && (
            <Card className="h-[300px] md:h-[400px] flex flex-col mt-4 md:mt-6">
              <CardHeader className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 text-sm md:text-base">
                <span>🌊 Form Wave Analysis (FFT)</span>
                <span className={`px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-bold ${waveAnalysis.waveDirection === 'RISING' ? 'bg-green-500 text-white' :
                  waveAnalysis.waveDirection === 'FALLING' ? 'bg-red-500 text-white' :
                    'bg-gray-400 text-white'
                  }`}>
                  {waveAnalysis.waveDirection === 'RISING' ? '📈 RISING' :
                    waveAnalysis.waveDirection === 'FALLING' ? '📉 FALLING' : '➡️ STABLE'}
                </span>
              </CardHeader>
              <CardContent className="flex-1 w-full min-h-[200px] relative p-0">
                <div className="absolute inset-0 p-4" style={{ minHeight: '180px' }}>
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                    <LineChart data={waveAnalysis.signal.map((v, i) => ({ match: i + 1, actual: v, wave: waveAnalysis.reconstructed[i] }))}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="match" label={{ value: 'Match #', position: 'bottom' }} />
                      <YAxis domain={[-1.5, 1.5]} label={{ value: 'Form', angle: -90, position: 'left' }} />
                      <Tooltip />
                      <Legend />
                      <ReferenceLine y={0} stroke="#888" />
                      <Line type="monotone" dataKey="actual" stroke="#004d40" strokeWidth={2} dot={{ r: 4 }} name="Actual Results" />
                      <Line type="monotone" dataKey="wave" stroke="#f59e0b" strokeWidth={3} dot={false} name="Underlying Wave" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
              <div className="px-6 pb-4 text-sm text-gray-600">
                <strong>Dominant Cycles:</strong> {waveAnalysis.dominantFrequencies.map(f =>
                  `Every ${f.period.toFixed(1)} matches (strength: ${(f.amplitude * 100).toFixed(0)}%)`
                ).join(' | ')}
              </div>
            </Card>
          )}
